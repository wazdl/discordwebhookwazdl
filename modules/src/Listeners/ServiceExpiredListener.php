...
class ServiceExpiredListener
{
    public function handle(object $event): void
    {
        try {
            ...
            $fields[] = ['name' => '⏱️ Date expiration', 'value' => now()->format('d/m/Y H:i'), 'inline' => false];

            DiscordWebhookService::send(
                'service_expired',
                '❌ Service expiré / supprimé',
                $fields,
                'Un service a expiré ou a été résilié.'
            );
        } catch (\Throwable $e) {
            // Fail silently
        }
